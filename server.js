...
app.get('/get-comments/:articleName', function (req, res) {
   ...
   // return a response with the results
   pool.query(
'SELECT comment.*, "user".username FROM comment, "user" WHERE article.title = $1 AND article.id = comment.article_id AND comment.user_id = "user".id ORDER BY comment.timestamp DESC', [req.params.articleName], function (err, result) {
      if (err) {
          res.status(500).send(err.toString());
      } else {
          res.send(JSON.stringify(result.rows));
